...
import { getLanguage, normalizeTextCommand, parseStartPayload } from '../../services/parser.js';
...
const getLocalizedText = (content: any, lang: string) => {
  ...
  if (lang === 'RU' && content.text_ru) return content.text_ru;
  return content.text || '';
};
...
export class ScenarioEngine {
  static async handleUpdate(bot: BotRuntime, session: any, update: any): Promise<boolean> {
    ...
    // --- Channel/Group Discovery Handling ---
    if (update.my_chat_member) {
      const { chat, new_chat_member } = update.my_chat_member;
      // If bot is added as administrator or member
      if (new_chat_member.status === 'administrator' || new_chat_member.status === 'member') {
        const chatTitle = chat.title || 'Unknown Channel';
        console.log(`[Bot] Added to chat: ${chat.id} (${chatTitle})`);

        // Log a system message to register this destination in BotMessage
        // This ensures it appears in the /destinations list
        await prisma.botMessage.create({
          data: {
            botId: bot.id,
            chatId: String(chat.id),
            direction: 'SYSTEM',
            text: `Bot added to chat: ${chatTitle}`,
            payload: { chat, from: update.my_chat_member.from }
          }
        }).catch(e => console.error('Failed to log new chat:', e));
      }
      return true; // Stop processing, no scenario for this
    }

    if (update.channel_post) {
      // Also capture channel posts to register the channel if missed
      const chat = update.channel_post.chat;
      const chatTitle = chat.title || 'Unknown Channel';
      // We don't need to log every post, but if we want to ensure it's known:
      // The existing message logging might handle it if we processed it?
      // Currently ScenarioEngine might ignore channel_post if not explicitly handled
      // Let's just log it if we haven't seen it recently? 
      // For now, let's just log it to ensure it appears in destinations
      // We can rely on the fact that if we receive it, it's alive.
      // Although, persisting every channel post might spam BotMessage.
      // apiRoutes /destinations does DISTINCT ON chatId effectively via the map.
      // So logging one message is enough.
      // Let's rely on standard message processing? 
      // Standard processing uses `update.message`. `channel_post` is different field.
      // We should normalize it or just log it here.
      ...
      await prisma.botMessage.create({
        data: {
          ...
          chatId: String(chat.id),
          direction: 'INCOMING',
          text: update.channel_post.text || '[Channel Post]',
  ...
  // --- CAR ACTION CALLBACKS ---
  if(input.startsWith('car:')) {
  const parts = input.split(':');
  const action = parts[1]; // ADD_REQUEST or ADD_CATALOG
  const carId = parts[2];

  const tempCars = Array.isArray(vars.__tempResults) ? vars.__tempResults : [];
  let targetCar = tempCars.find((c: any) => c.canonicalId === carId || c.id === carId);

  // If not found in simple temp, maybe it's in DB?
  if (!targetCar && carId && !carId.startsWith('autoria_')) {
    targetCar = await prisma.carListing.findUnique({ where: { id: carId } }).catch(() => null);
    if (targetCar) targetCar = mapDbCar(targetCar);
  }

  if (targetCar) {
    if (action === 'add_request') {
      const reqId = await ScenarioEngine.resolveRequestId(vars);
      if (reqId) {
        const mapped = mapVariantInput({
          title: targetCar.title,
          url: targetCar.sourceUrl,
          sourceUrl: targetCar.sourceUrl,
          source: targetCar.source || 'EXTERNAL',
          status: 'SELECTED',
          price: targetCar.price,
          year: targetCar.year,
          thumbnail: targetCar.thumbnail,
          specs: targetCar.specs
        });
        // Check duplicate
        const existing = await prisma.requestVariant.findFirst({
          where: { requestId: reqId, sourceUrl: targetCar.sourceUrl }
        });

        if (!existing) {
          await prisma.requestVariant.create({
            data: { ...mapped, requestId: reqId }
          });
          await sendMessage(bot, chatId, lang === 'UK' ? '✅ Додано до запиту!' : lang === 'RU' ? '✅ Добавлено в запрос!' : '✅ Added to request!');
        } else {
          await sendMessage(bot, chatId, lang === 'UK' ? '⚠️ Вже є в запиті.' : lang === 'RU' ? '⚠️ Уже в запросе.' : '⚠️ Already in request.');
        }
      } else {
        await sendMessage(bot, chatId, lang === 'UK' ? '⚠️ Спочатку створіть запит.' : lang === 'RU' ? '⚠️ Сначала создайте запрос.' : '⚠️ Create a request first.');
      }
      await answerCallback(bot, update.callback_query.id, 'Done');
      return true;
    }

    if (action === 'add_catalog') {
      // Upsert to inventory
      // If ID exists in DB, it's already there
      const exists = await prisma.carListing.findUnique({ where: { id: targetCar.canonicalId } });
      if (!exists) {
        // Map to DB input
        await prisma.carListing.create({
          data: {
            id: targetCar.canonicalId,
            title: targetCar.title,
            price: targetCar.price?.amount || 0,
            currency: targetCar.price?.currency || 'USD',
            year: targetCar.year,
            mileage: targetCar.mileage,
            thumbnail: targetCar.thumbnail,
            mediaUrls: targetCar.mediaUrls || [],
            sourceUrl: targetCar.sourceUrl,
            source: targetCar.source || 'EXTERNAL',
            status: 'AVAILABLE',
            specs: targetCar.specs || {}
        ...
        await sendMessage(bot, chatId, lang === 'UK' ? '✅ Збережено в каталог!' : lang === 'RU' ? '✅ Сохранено в каталог!' : '✅ Saved to catalog!');
      } else {
        await sendMessage(bot, chatId, lang === 'UK' ? '⚠️ Вже в каталозі.' : lang === 'RU' ? '⚠️ Уже в каталоге.' : '⚠️ Already in catalog.');
      }
      await answerCallback(bot, update.callback_query.id, 'Done');
      return true;
    }
  } else {
    await answerCallback(bot, update.callback_query.id, 'Car not found');
    return true;
  }
}
// ----------------------------
...
// Helper to find request ID
const resolveRequestId = async (vars: any) => {
  if (vars.requestId) return vars.requestId;
  // Search recent request by chat
  const last = await prisma.b2bRequest.findFirst({
    where: { clientChatId: chatId },
    orderBy: { createdAt: 'desc' }
  });
  return last?.id;
};
...
return true;
  ...
  switch (node.type) {
    ...
    case 'MESSAGE':
      ...
      break;
    case 'MESSAGE': {
      const rawText = getLocalizedText(node.content || {}, lang);
      const text = formatWithVars(rawText, vars);
      await sendMessage(bot, session.chatId, text);
      if (node.nextNodeId) await this.executeNode(bot, session, vars, history, scenario, node.nextNodeId, isBack);
      else {
        delete vars.__activeScenarioId;
    ...
    case 'GALLERY': {
      const rawText = getLocalizedText(node.content || {}, lang);
      await sendMessage(bot, session.chatId, formatWithVars(rawText, vars));
    ...
    case 'CHANNEL_POST': {
      ...
      if (scheduledAt) {
        ...
        // Use IntegrationService for centralized sending
        try {
          await integrationService.publishTelegramChannelPost({
          ...
          console.error('Scenario CHANNEL_POST failed via integrationService, falling back to direct send');
          // Fallback if IntegrationService fails (though it shouldn't if token is valid)
