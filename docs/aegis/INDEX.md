# Aegis Index

| Date | Kind | Path | Title | Status |
| --- | --- | --- | --- | --- |
| 2026-05-27 | baseline | `docs/aegis/baseline/2026-05-27-initial-baseline.md` | Initial Cartie Baseline Snapshot | current snapshot |
| 2026-05-27 | spec | `docs/aegis/specs/2026-05-27-meta-tracking-capi-design.md` | Meta Tracking, CAPI, and Telegram Attribution Design | review pending |
| 2026-05-27 | plan | `docs/aegis/plans/2026-05-27-meta-tracking-capi-implementation-plan.md` | Meta Tracking, CAPI, and Telegram Attribution Implementation Plan | review pending |
| 2026-05-27 | adr | `docs/aegis/adr/2026-05-27-attribution-session-owner.md` | AttributionSession Owns Direct-To-Telegram Attribution | accepted for implementation branch |
| 2026-05-27 | audit | `docs/audit/cartie_bot_miniapp_scenario_audit_2026-05-27.md` | Cartie Bot/MiniApp Scenario Audit | implemented in remediation branch |
| 2026-05-28 | plan | `docs/aegis/plans/2026-05-28-miniapp-scenario-audit-remediation-and-meta-dataset-plan.md` | MiniApp Scenario Audit Remediation And Meta Dataset Plan | implemented in remediation branch |
| 2026-06-01 | work | `docs/aegis/work/2026-06-01-miniapp-audit-remediation/` | MiniApp Audit Remediation Work Trail | handoff ready |
| 2026-06-01 | adr | `docs/aegis/adr/2026-06-01-miniapp-signed-read-and-meta-gate-owners.md` | MiniApp Signed Read And Meta Gate Owners | accepted for implementation branch |
| 2026-06-18 | adr | `docs/aegis/adr/2026-06-18-vehicle-taxonomy-local-snapshot-owner.md` | Vehicle Taxonomy Local Snapshot Owner | accepted for implementation branch |
| 2026-06-18 | spec | `docs/aegis/specs/2026-06-18-vehicle-taxonomy-source-design.md` | Vehicle Taxonomy Source Design | approved for implementation planning |
| 2026-06-18 | plan | `docs/aegis/plans/2026-06-18-vehicle-taxonomy-source-implementation-plan.md` | Vehicle Taxonomy Source Implementation Plan | implementation slice verified |
| 2026-06-18 | work | `docs/aegis/work/2026-06-18-vehicle-taxonomy-source/` | Vehicle Taxonomy Source Work Trail | implementation slice verified |
