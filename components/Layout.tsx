...
export const Layout: React.FC<LayoutProps> = ({ children, user, onLogout }) => {
  ...
  useEffect(() => {
      const load = async () => {
          ...
          setNavItems((settings.navigation || []).sort((a: any, b: any) => a.order - b.order));
          ...
          {visibleNavItems.map(item => {
              ...
              return (
                <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3.5 text-[15px] font-medium transition-all rounded-xl relative overflow-hidden group ${
                    active 
