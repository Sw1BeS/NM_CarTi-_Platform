...
import { Plus, List as ListIcon, LayoutGrid, Search as SearchIcon, Filter, DollarSign, Calendar, ChevronRight, MapPin } from 'lucide-react';
...
export const RequestList: React.FC = () => {
  ...
  const loadRequests = async () => setRequests(await Data.getRequests());

  const filtered = requests.filter(r => r.title.toLowerCase().includes(search.toLowerCase()) || r.publicId.toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="space-y-8 h-[calc(100vh-140px)] flex flex-col">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 shrink-0">
        <div>
            ...
            <p className="text-sm text-[var(--text-secondary)] mt-1">Sourcing pipeline</p>
        </div>
                 ...
                 {[RequestStatus.NEW, RequestStatus.IN_PROGRESS, RequestStatus.READY_FOR_REVIEW, RequestStatus.PUBLISHED].map(colStatus => (
                     <div key={colStatus} className="w-96 shrink-0 flex flex-col bg-[var(--bg-input)] rounded-2xl border border-[var(--border-color)] h-full backdrop-blur-sm">
                         <div className="p-5 flex justify-between items-center border-b border-[var(--border-color)]">
                             ...
                             <span className="bg-[var(--bg-panel)] px-3 py-1 rounded text-xs font-bold text-[var(--text-primary)] shadow-sm border border-[var(--border-color)]">{filtered.filter(r => r.status === colStatus).length}</span>
                         </div>
                         <div className="flex-1 overflow-y-auto p-4 space-y-4">
                             {filtered.filter(r => r.status === colStatus).map(r => (
