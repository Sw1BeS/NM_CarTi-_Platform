...
describe('SalesDrive inbound webhook service', () => {
  ...
  it('enriches AdsQuiz SalesDrive webhooks when Cartie token is carried in utm_content composite text', async () => {
    vi.stubEnv('SALESDRIVE_B2C_WEBHOOK_ACCOUNT_ALLOWLIST', 'cartie');
    vi.stubEnv('SALESDRIVE_B2C_WEBHOOK_FORM_ALLOWLIST', '1');
    vi.stubEnv('SALESDRIVE_B2C_META_STATUS_MAP', JSON.stringify([
    ...
    const result = await handleSalesDriveWebhook({
      ...
      body: {
        data: {
          ...
          formId: 1,
          statusId: 2,
          updateAt: '2026-06-16T10:05:00Z',
          utm_content: 'creative_a|cartie_token_quizTOKEN_1234567890__term_120245481203690526',
          utm_term: '120245481203690526',
          contacts: [{
            phone: '+38 (063) 505-52-52',
            fName: 'Lead',
    ...
    expect(result).toMatchObject({ ok: true, sent: true, eventName: 'QualifiedLead' });
    expect(prismaMock.attributionSession.findUnique).toHaveBeenCalledWith({
      where: { token: 'quizTOKEN_1234567890' }
    });
    expect(trackB2CBotCrmLifecycleEventMock).toHaveBeenCalledWith(
      null,
      'QualifiedLead',
