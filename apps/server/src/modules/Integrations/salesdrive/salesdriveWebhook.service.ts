...
const readAttributionTokenFromUtmText = (value: unknown) => {
  ...
  if (!text) return '';
  return toText(
    text.match(/\bcartie_token_([A-Za-z0-9_-]{12,80})\b/i)?.[1]
    || text.match(/\bcartie_attribution_([A-Za-z0-9_-]{12,80})\b/i)?.[1]
    || text.match(/\bcartie_attribution_token[:=]([A-Za-z0-9_-]{12,80})\b/i)?.[1]
  );
};
...
export const parseSalesDriveWebhookPayload = (body: Record<string, unknown>): ParsedSalesDriveStatus => {
  ...
  const attributionToken = toText(
    ...
    || readAttributionTokenFromUtmText(pickFromRecords([body, client, order, data, firstContact], ['utm_term', 'utmTerm']))
    || readAttributionTokenFromUtmText(readCommentLineValue(comment, 'utm_term'));
