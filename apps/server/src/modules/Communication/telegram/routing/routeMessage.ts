...
const handleClientLead = async (ctx: PipelineContext, text: string) => {
  ...
  if (state === 'CL_MINIAPP_CONTACT') {
    ...
    const miniappInterestDraft = (vars.miniappInterestDraft || {}) as Record<string, any>;
    ...
    const requestedIds = Array.isArray(miniappInterestDraft.carIds)
      ? miniappInterestDraft.carIds.map((item) => String(item || '').trim()).filter(Boolean)
      : [];
    const selectedCars = requestedIds.length
    ...
    const selectedCarsOrdered = requestedIds.map((id) => selectedCarsMap.get(id)).filter((item): item is typeof selectedCars[number] => Boolean(item));
    const requestTitle = String(
      miniappInterestDraft.title
    ...
    const leadResult = await createOrMergeLead({
      ...
      payload: {
        ...
        carId: miniappInterestDraft.carId || selectedCarsOrdered[0]?.id || undefined,
        carIds: requestedIds.length ? requestedIds : undefined,
        selectedCars: selectedCarsOrdered,
        tracking: miniappInterestDraft.meta || undefined
      },
      leadType: 'BUY',
      createRequest: true,
      requestData: {
        ...
        description: [
          ...
          selectedCarsOrdered.length > 1 ? `Обрані авто: ${selectedCarsOrdered.map((car) => car.title).join(', ')}` : null
        ].filter(Boolean).join('\n'),
        language: 'UK'
      }
    }, ctx.bot.config);

    await sendMessage(ctx, leadResult.isDuplicate
      ? '✅ Інтерес уже був зафіксований, ми оновили звернення.'
      : '✅ Дякуємо! Менеджер звʼяжеться з вами найближчим часом.', { remove_keyboard: true });

    if (ctx.bot.adminChatId) {
      const userLink = telegramUsername
        ? `https://t.me/${telegramUsername.replace(/^@/, '')}`
        : `tg://user?id=${tgUserId}`;
      const selectedCarsText = selectedCarsOrdered.length
        ? selectedCarsOrdered.map((car, index) => `${index + 1}. ${car.title}${car.year ? ` ${car.year}` : ''}`).join('\n')
        : '—';
      const adminLines = [
        ...
        `Авто: ${requestTitle}`,
        `Обрані авто:\n${selectedCarsText}`,
        leadResult.request ? `Request ID: ${leadResult.request.publicId || leadResult.request.id}` : null
      ].filter(Boolean);
