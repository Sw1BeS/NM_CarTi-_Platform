...
describe('tracking redirect routes', () => {
  ...
  it('creates a session and redirects to an allowlisted AdsQuiz URL', async () => {
    const createSession = vi.fn().mockResolvedValue({
      ...
      redirectUrl: 'https://cartieua.adsquiz.io/?cartie_attribution_token=quizTOKEN_1234567890&fbclid=QuizClick',
      snapshot: {},
      cookies: {
    ...
    expect(res.status).toBe(302);
    expect(res.headers.location).toBe('https://cartieua.adsquiz.io/?cartie_attribution_token=quizTOKEN_1234567890&fbclid=QuizClick');
    expect(createSession).toHaveBeenCalledWith(expect.objectContaining({
      destination: 'adsquiz_usa',
      redirectUrl: 'https://cartieua.adsquiz.io/',
      appendAttributionParams: true,
      source: 'meta',
      query: expect.objectContaining({
        fbclid: 'QuizClick'
