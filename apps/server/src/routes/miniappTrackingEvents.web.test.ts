...
describe('MiniApp web tracking event names', () => {
    ...
    it('removes Telegram auth query params but keeps campaign params', () => {
        expect(sanitizeMiniAppEventSourceUrl(
            'https://cartie.test/p/app/cartie?utm_source=meta&tgWebAppData=secret&user=secret&initData=raw&init_data=raw2&telegramInitData=raw3&telegram_init_data=raw4&kbAuth=secret&keyboardAuth=secret2&fbclid=ClickId'
