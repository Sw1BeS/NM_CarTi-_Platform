...
describe('validateEnv', () => {
  ...
  it('accepts attribution redirect env keys and keeps redirect disabled by default', () => {
    ...
    vi.stubEnv('ATTRIBUTION_WEB_ALLOWLIST', 'adsquiz_usa=https://cartieua.adsquiz.io/');
  ...
  it('accepts main quiz Meta CAPI env keys without enabling them by default', () => {
    vi.stubEnv('NODE_ENV', 'development');
    vi.stubEnv('META_MAIN_QUIZ_DATASET_ID', '1813442132635673');
    vi.stubEnv('META_MAIN_QUIZ_DESTINATION_KEY', 'main_quiz_adsquiz');
    vi.stubEnv('META_MAIN_QUIZ_ACCESS_TOKEN', 'env-only-main-token');
    vi.stubEnv('META_MAIN_QUIZ_CAPI_ENABLED', 'false');
    vi.stubEnv('META_MAIN_QUIZ_TEST_MODE', 'true');
    vi.stubEnv('META_MAIN_QUIZ_TEST_EVENT_CODE', 'TEST_MAIN');
    vi.stubEnv('META_DUAL_SALESDRIVE_TARGETS', 'b2c_bot,main_quiz');

    const env = validateEnv();

    expect(env.META_MAIN_QUIZ_DATASET_ID).toBe('1813442132635673');
    expect(env.META_MAIN_QUIZ_DESTINATION_KEY).toBe('main_quiz_adsquiz');
    expect(env.META_MAIN_QUIZ_ACCESS_TOKEN).toBe('env-only-main-token');
    expect(env.META_MAIN_QUIZ_CAPI_ENABLED).toBe('false');
    expect(env.META_MAIN_QUIZ_TEST_MODE).toBe('true');
    expect(env.META_MAIN_QUIZ_TEST_EVENT_CODE).toBe('TEST_MAIN');
    expect(env.META_DUAL_SALESDRIVE_TARGETS).toBe('b2c_bot,main_quiz');
  });
});
